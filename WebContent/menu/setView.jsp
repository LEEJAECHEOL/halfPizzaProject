...
<%@ include file="../layouts/header.jsp" %>
        ...
        <div class="container">
            ...
            <div class="choice-menu-detail">
                ...
                <div class="choice-menu-detail-right">
                    ...
                    <div class="choice-menu-detail-right-bottom">
						<h3>스파게티 변경선택</h3>
						<div class="choice-menu-detail-right-bottom-list choice-seupageti">
						<c:choose>
						  	<c:when test="${optional!=null}">
								<c:forEach var="opt" items="${optional}">
									<c:if test="${opt.gubun eq 'seupageti'}">
										<span data-id=${opt.id} data-price=${opt.price}>${opt.title}</span>
									</c:if>
								</c:forEach>
						  	</c:when>
    ...
    <script>
	    ...
	    document.querySelector('.choice-seupageti').addEventListener('click', function(e){
	    	if(e.target && e.target.tagName==="SPAN"){
	    		let _span = e.target;
	    		let _totalPrice = document.querySelector('#totalPrice');
				if(_span.classList.contains("selected")){
					_totalPrice.dataset.total = Number(_totalPrice.dataset.total) - Number(document.querySelector('.menu-request p.seupageti').dataset.price);
					document.querySelector('.menu-request p.seupageti').remove();
					document.querySelector('.choice-seupageti span.selected').classList.remove('selected');
					_totalPrice.textContent = moneyComma(_totalPrice.dataset.total);
					return;
				}
	    		if(document.querySelector('.choice-seupageti span.selected')){
	    			document.querySelector('.choice-seupageti span.selected').classList.remove('selected');
	    		}
	    		_span.classList.add('selected');
	    		let _request = document.querySelector('.menu-request');
				...
				if(document.querySelector('.menu-request p.seupageti')){
					_totalPrice.dataset.total = Number(_totalPrice.dataset.total) - Number(document.querySelector('.menu-request p.seupageti').dataset.price);
					document.querySelector('.menu-request p.seupageti').remove();
				}
				let _content = "<p class='seupageti' data-id='" + _id + "' data-price='" + _price + "'><span>+ " + _text + "</span><b>" + moneyComma(_price) + "</b></p> ";
				_request.insertAdjacentHTML("beforeend", _content);
				_totalPrice.dataset.total = Number(_totalPrice.dataset.total) + Number(_price);
				_totalPrice.textContent = moneyComma(_totalPrice.dataset.total);
	    	}
